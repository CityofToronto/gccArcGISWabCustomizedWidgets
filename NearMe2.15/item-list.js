...
define([
  ...
  return declare([_WidgetBase, Evented], {
    ...
    postCreate: function () {
      ...
      this.configSearchableLayers = this.config.searchLayers;
    ...
    _showFilteredFeaturesOnLoad: function (features, layerID) {
      ...
      if (this.config.selectedSearchLayerOnly) {
        ...
        if (maxFeatureLength) {
          ...
          filter = "(" + this._selectedLayer.objectIdField + ' in (' + featureIds + ')' + ")" + (queryDef?" AND (" + queryDef + ")":"");
    ...
    **/
    showAllLayers: function (honorLayersVisibility) {
      ...
      for (i = 0; i < this.config.searchLayers.length; i++) {
        ...
        if (honorLayersVisibility) {
          showLayer = this.config.searchLayers[i].visibility || this.map._layers[this.config.searchLayers[i].id].visible;
        } else {
          showLayer = true;
        }
        if (this.config.selectedSearchLayerOnly &&
    ...
    /* rui's function to get all visible layers set by feature filter*/ 
    getVisibleLayers: function() {  
      var self = this 
      self.searchableLayers = []  
      //self.config.searchableLayers = configSearchableLayers 
      self.map.webMapResponse.itemInfo.itemData.operationalLayers.filter(item => {                
        if (item.layerObject && item.layerObject != undefined) {  
          if (item.layerObject.visible) { 
            self.searchableLayers.push(self.configSearchableLayers.filter(i => i.id == item.layerObject.id)[0]) 
          }   
        } 
      })  
      console.log(self.searchableLayers)  
      this.config.searchLayers = self.searchableLayers  
    },  
